...
def fit_numpyro(
    ...
    if inference == "nuts":
        ...
        nchains = 2 if jax.local_device_count() >= 2 else 1
        mcmc = MCMC(kernel, num_warmup=1000, num_samples=1500, num_chains=nchains, progress_bar=True)
...
def _subset_posterior(posterior: Dict[str, jnp.ndarray], n_draws: int, seed: int) -> Dict[str, jnp.ndarray]:
    """Deterministically subsample posterior draws (for NUTS Predictive)."""
    if n_draws is None:
        return posterior
    # number of stored posterior draws
    m = int(next(iter(posterior.values())).shape[0])
    if n_draws >= m:
        return posterior
    rng = np.random.default_rng(seed)
    idx = rng.choice(m, size=int(n_draws), replace=False)
...
def predict_logy_draws(
    ...
    """
    Draw posterior predictive samples of log(target) for each row in X.

    Returns array shaped (N, draws) robustly for both SVI and NUTS.
    For NUTS, we subsample posterior draws to exactly n_draws (if possible)
    to avoid Predictive defaulting to all stored draws.
    """
    rng = jax.random.PRNGKey(seed)
    ...
    if fit.inference == "nuts":
        # Subsample posterior draws to the requested n_draws (deterministic by seed)
        posterior_sub = _subset_posterior(fit.posterior, n_draws=n_draws, seed=seed)
        pred = Predictive(model_fn, posterior_samples=posterior_sub)
        out = pred(rng, x=x, y=None, **model_kwargs)
        ydraw = np.array(out["y"])  # typically (draws, N)
    else:
    ...
    # Robust orientation to (N, draws)
    N = X.shape[0]
    if ydraw.ndim != 2:
        raise ValueError(f"Unexpected Predictive output shape for y: {ydraw.shape}")

    if ydraw.shape[0] == N and ydraw.shape[1] != N:
        # already (N, draws)
        pass
    elif ydraw.shape[1] == N and ydraw.shape[0] != N:
        # transpose from (draws, N) -> (N, draws)
        ydraw = ydraw.T
    elif ydraw.shape[0] == N and ydraw.shape[1] == N:
        # ambiguous (rare); keep as-is
        pass
    else:
        raise ValueError(f"Could not align Predictive draws to (N, draws). Got {ydraw.shape}, expected one dim == N={N}.")
